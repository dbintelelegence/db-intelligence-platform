...
export function TabNavigation() {
  ...
  const { clusters, issues } = useDashboardContext();

  // Live counts derived from backend data
  ...
  const tabs = [
    ...
    },
    {
      ...
      path: '/databases',
      icon: Database,
      badge: clusters.length || undefined,
    },
    {
      name: 'Issues & Anomalies',
      path: '/issues',
      icon: AlertTriangle,
      badge: criticalIssuesCount || undefined,
  ...
  // Live counts by cloud provider
  const awsCount   = clusters.filter(db => db.cloud === 'aws').length;
  const gcpCount   = clusters.filter(db => db.cloud === 'gcp').length;
  const azureCount = clusters.filter(db => db.cloud === 'azure').length;

  // Live counts by database type
  const postgresCount      = clusters.filter(db => db.type === 'postgres').length;
  const mysqlCount         = clusters.filter(db => db.type === 'mysql').length;
  const mongoCount         = clusters.filter(db => db.type === 'mongodb').length;
  const redisCount         = clusters.filter(db => db.type === 'redis').length;
  const dynamoCount        = clusters.filter(db => db.type === 'dynamodb').length;
  const auroraCount        = clusters.filter(db => db.type === 'aurora').length;
  const elasticsearchCount = clusters.filter(db => db.type === 'elasticsearch').length;
                ...
                <div className="ml-4 mt-2 flex flex-col gap-4 border-l-2 border-muted pl-4">
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-2 py-1">
                      ...
                      <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">By Cloud</h3>
                    </div>
                    <div className="flex flex-col gap-1">
                      {cloudGroups.map((group) => (
                        <Link
                          key={group.path}
                          to={group.path}
                          className="flex items-center justify-between px-3 py-2 text-sm transition-colors rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground"
                        >
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-2 py-1">
                      ...
                      <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">By Type</h3>
                    </div>
                    <div className="flex flex-col gap-1">
